...
double distanceMeters = 15;               //!< Distance (m) of the base station from the centre of the circle of deployment 
double nt = 8e7;    
double prad = 3e-6;
double pcktSize = 50; 
...
main(int argc, char* argv[])
{
    ...
    cmd.AddValue("distance", "Distance (m) of the deployment of the base station:", distanceMeters);
    cmd.AddValue("N_T", "Particle Density:", nt);
    cmd.AddValue("prad", "Particle Radius:", prad);
    cmd.AddValue("pcktSize", "Packet Size in Bytes:", pcktSize);
    ...
    Ptr<MarsPropagationLossModel> loss = CreateObject<MarsPropagationLossModel>();
    loss->SetPathLossExponent(3);
    loss->SetReference(1, 7.7);
    loss->SetDistance(10000.0);
    loss->SetLambda(299792458.0/freq);
    loss->SetEpsilonReal(2.9038);
    loss->SetEpsilonImm(0.1278);
    loss->SetParticleRadius(prad);
    loss->SetN_T(nt);
    ...
    // Make it so that nodes are at a certain height > 0
    allocator->Add(Vector(0.0, 0.0, distanceMeters));
